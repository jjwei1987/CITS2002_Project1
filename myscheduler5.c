...
void execute_commands(void)
{
    ...
    // Declare and initialize 'i' here
    int i = 0;

    // Step 2: Loop through the commands
    while (i < ncommands || !is_queue_empty(&reexecution_queue))
    {
        // Check if there are no commands in the reexecution queue
        if (is_queue_empty(&reexecution_queue))
        {
            // Calculate the total time for the command to run
            int total_run_time = 0;
            for (int j = 0; j < command_list[i].nsyscalls; j++) {
                total_run_time += command_list[i].syscalls[j].run_time;
            }
                
            // Calculate the remaining execution time
            int remaining_execution_time = total_run_time - TOTAL_PROCESS_TIME_ON_CPU;
            command_list[i].syscalls[0].run_time = remaining_execution_time;
        ...
        {
            // Get the first command from the reexecution_queue
            char front_command[MAX_COMMAND_NAME];
            strcpy(front_command, reexecution_queue.queue_name);
            add_to_queue(&ready_queue, front_command, "ready");
            GLOBAL_CLOCK += TIME_CORE_STATE_TRANSITIONS; // Increment for the move to ready queue
            // Print information about the command being moved to running queue
         ...
         GLOBAL_CLOCK += TIME_CONTEXT_SWITCH; // Increment for the move to running queue
        ...
        printf("Running Queue: %s\n", ready_queue.queue_name);
         ...
         {
             // Move it to the blocked queue, block time is the time
             ...
             // Print information about the command being moved to blocked queue
             ...
             if (strcmp(command_list[i].syscalls[0].syscall_name, "sleep") == 0)
             {
                 ...
                 if (sleep_time > 0)
                 {
                     ...
                     printf("Command %s in sleep state %i\n", command_list[i].command_name, GLOBAL_CLOCK);
                         ...
                         // Check if there are any commands in the blocked queue
                         if (!is_queue_empty(&blocked_queue))
                         {
                             // Get the front command from the blocked queue
                             char front_command[MAX_COMMAND_NAME];
                             strcpy(front_command, blocked_queue.queue_name);
                             
                             // Change the sleep syscall's duration to 0
                             command_list[i].syscalls[0].run_time = 0;
                             // Update the reexecution queue with the updated command information
                             add_to_queue(&reexecution_queue, front_command, "reexecution");
                             
                             // Remove the command from the blocked queue
                             move_out_of_queue(&blocked_queue, front_command);
             ...
             // Increment i here if no reexecution is needed
             i++;
         }
     }
    ...
    // If the command run_time is over the time quantum, move it to the ready queue, clock + 10
    ...
    // Check next command - check if the command is spawn
    if (strcmp(command_list[i].syscalls[0].syscall_name, "spawn") == 0)
    {
        // If it is, create a new process for the child command and add the child command to the ready queue, clock + 10
        add_to_queue(&ready_queue, command_list[i].syscalls[0].child_command_name,  "ready");
        printf("Ready Queue: %s\n", ready_queue.queue_name);
        GLOBAL_CLOCK += TIME_CORE_STATE_TRANSITIONS;
    }
    
    // Check next command - check if the command is exit
    if (strcmp(command_list[i].syscalls[0].syscall_name, "exit") == 0)
    {
        // If it is, move the command out of the running queue, clock + 0
