...
void move_out_of_queue(struct queue *q, char *command)
{
    ...
    if (!is_queue_empty(q))
    {
        if(q->front == q->end) // If there is only one item in the queue, reset the front and end index
        {
            q->front = -1;
            q->end = -1;
        }
...
void execute_commands(void)
{
    ...
    for (int i = 0; i < ncommands; i++)
    {
        ...
        // if it is, then move it out of the running queue, TOTAL_PROCESS_TIME_ON_CPU not updated
        ...
        if (strcmp(command_list[i].syscalls[0].syscall_name, "sleep") == 0 || strcmp(command_list[i].syscalls[0].syscall_name, "wait") == 0)
        {
            ...
            if (strcmp(command_list[i].syscalls[0].syscall_name, "sleep") == 0)
            {
                ...
                if (sleep_time > 0)
                {
                    ...
                    add_to_queue(&blocked_queue, command_list[i].command_name, "ready");
                    
                }
                
                else if (strcmp(command_list[i].syscalls[0].syscall_name, "wait") == 0)
                {
                    // Check if the command's current syscall is "wait"
                    // if it is, then move it out of the running queue, TOTAL_PROCESS_TIME_ON_CPU not updated
                    // treat it as a sleep syscall
                    int remaining_execution_time = command_list[i].syscalls[0].run_time - command_list[i].total_execution_time;
                }
            }
        }
        ...
        else if (strcmp(command_list[i].syscalls[0].syscall_name, "spawn") == 0)
        {
            ...
            printf("Ready Queue: %s\n", ready_queue.queue_name);
            add_to_queue(&reexecution_queue, command_list[i].command_name, "reexecution");
            GLOBAL_CLOCK += TIME_CORE_STATE_TRANSITIONS;
        }
        ...
        // if it is, then move it out of the running queue, TOTAL_PROCESS_TIME_ON_CPU not updated
        // loop all the devices to find the device that matches the syscall's device name
        // Need to set the blocked time of process to the time taken for the read to complete.
        // blocked time will be read speed devided by the argument in syscalls + TIME_ACQUIRE_BUS
        for(int j = 0; j < ndevices; j++)
        {
            if(strcmp(command_list[i].syscalls[0].syscall_name, "read") == 0)
            {
                if(strcmp(command_list[i].syscalls[0].device_name_of_syscall, devices[j].device_name) == 0)
                {
                    int blocked_time = devices[j].read_speed / command_list[i].syscalls[0].syscall_argument + TIME_ACQUIRE_BUS;
                    GLOBAL_CLOCK += blocked_time;
                    printf("Command %s blocked to ready %i\n", command_list[i].command_name, GLOBAL_CLOCK);
                    add_to_queue(&blocked_queue, command_list[i].command_name, "ready");
        ...
        // check if the syscall is "write"
        // if it is, then move it out of the running queue, TOTAL_PROCESS_TIME_ON_CPU not updated
        // loop all the devices to find the device that matches the syscall's device name
        // Need to set the blocked time of process to the time taken for the read to complete.
        // blocked time will be write speed devided by the argument in syscalls + TIME_ACQUIRE_BUS
        for(int j = 0; j < ndevices; j++)
        {
            if(strcmp(command_list[i].syscalls[0].syscall_name, "write") == 0)
            {
                if(strcmp(command_list[i].syscalls[0].device_name_of_syscall, devices[j].device_name) == 0)
                {
                    int blocked_time = devices[j].write_speed / command_list[i].syscalls[0].syscall_argument + TIME_ACQUIRE_BUS;
                    GLOBAL_CLOCK += blocked_time;
                    printf("Command %s blocked to ready %i\n", command_list[i].command_name, GLOBAL_CLOCK);
                    add_to_queue(&blocked_queue, command_list[i].command_name, "ready");
                    printf("Blocked Queue: %s\n", blocked_queue.queue_name);
