...
//Define a structure for a process... not sure if needed
typedef struct {
    // ...
} process; // Note: It appears 'process' is similar to a 'command' in this context.*/
...
typedef struct {
    ...
    int front;
    int end;
    command processes[MAX_RUNNING_PROCESSES];  // Change 'process' to 'command' here
} queue;
...
{
    // There is more to checking that a queue is full but this is a fair start.
    return ((q->front == 0) && (q->end == MAX_RUNNING_PROCESSES - 1));
}


// Add a command/process to any one of the command/process queues. It could return 0 if successful, or a non-zero value to indicates a type of failure.

int add_to_queue(queue *q, command *cmd) {
    if (is_queue_full(q)) {
        ...
        q->processes[q->end] = *cmd; // Added process to queue
        return 0; // Success
    }
}

void addToReadyQueue(int command)
{
    for (int i = 0; i < ncommands; i++) {
        //  Append a command to the ready_queue array using the above generic function
        switch (add_to_queue(&ready_queue, &command_list[i])) { // Pass a pointer to the command
            case 0:
...
void read_commands(char argv0[], char filename[])
{
    ...
    if (commands != NULL)
    {
        ...
        while (fgets(command_line, BUFSIZE, commands) != NULL)
        {
            ...
            {
                // Parse the command name and store it in the current_command structure
                if (sscanf(command_line, "%s", current_command.command_name) == 1)
                {
                    printf("Command %i %s\n", ncommands, current_command.command_name);
                    current_command.nsyscalls = 0; // Initialise syscall count for this command
