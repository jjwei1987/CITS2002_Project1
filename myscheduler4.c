...
int add_to_queue(struct queue *q, char *command, const char *queue_type)
{
    ...
    else
    {
        ...
        // Customize the print statement based on the queue type
...
void execute_commands(void)
{
    ...
    for (int i = 0; i < ncommands; i++)
    {
        ...
        GLOBAL_CLOCK += 5;//increment for the move to ready queue
        ...
        if (strcmp(command_list[i].syscalls[0].syscall_name, "sleep") == 0 || strcmp(command_list[i].syscalls[0].syscall_name, "wait") == 0)
        {
            ...
            GLOBAL_CLOCK += 10; //increment for the move to blocked queue
            // Print information about the command being moved to running queue
            ...
            // Check if it's time to unblock the command
            while (sleep_time > 0)
            {
                ...
                if (!is_queue_empty(&blocked_queue))
                {
                    ...
                    // Check if the front command has completed its sleep time
                    if (strcmp(front_command, command_list[i].command_name) == 0 && GLOBAL_CLOCK >= sleep_time)
                    {
                        // Move it back to the ready queue
                        add_to_queue(&ready_queue, front_command, "ready");
                        move_out_of_queue(&blocked_queue, front_command);
                        printf("Ready Queue: %s\n", ready_queue.queue_name);
                        break; // Exit the loop since we've unblocked the command
                    }
                }
